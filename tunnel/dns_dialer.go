...
func (d *DNSDialer) DialContext(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	if metadata.NetWork == C.TCP {
		...
		conn = statistic.NewTCPTracker(conn, statistic.GetManagers(conn.Chains()), metadata, rule, 0, 0, false)
		...
		packetConn = statistic.NewUDPTracker(packetConn, statistic.GetManagers(packetConn.Chains()), metadata, rule, 0, 0, false)
